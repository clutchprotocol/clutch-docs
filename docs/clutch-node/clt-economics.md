...
`APP_REDEMPTION_FEE_USDT`, in micro-USDT, per deployment rather than a protocol constant. It defaults to **zero**, so a deployment that never sets it redeems at par. This testnet currently charges **$5.00**.

That figure is measured rather than chosen. On 2026-09-10 a TRC-20 USDT transfer into an address that held no USDT yet burned 130,285 units of energy, at a `getEnergyFee` of 100 sun per unit — 13.03 TRX, about $4.43 at a TRX price of $0.34, plus roughly $0.12 of bandwidth once the day's free 600 bytes are spent. A payout is that expensive shape whenever the recipient's Tron address is new to USDT, which the treasury does not get to control, so the fee covers the worst case rather than the average. A transfer into an address that already holds USDT costs half as much.

Two things bound the number. Below it, the smallest redemption the deployment allows: the treasury refuses a redemption that cannot cover its own fee, and that refusal reaches the user as a bare 502 rather than an explanation, so the minimum moved to **$10** when the fee moved to $5. Above it, nothing but the ratio — and at the $10 floor the fee is half of what the user asked for. That is what a $4.55 on-chain payout costs against a $10 redemption, not a margin the treasury keeps; it falls to 5% at $100. The two ways to improve it at the floor are a higher minimum with a higher cap to match, or staking TRX for energy instead of burning it, and both are decisions rather than settings.

Charging the mainnet number on a testnet, where Nile energy is faucet TRX and the cost is not real, is deliberate: the arithmetic a Withdraw panel shows here is then the arithmetic a mainnet user will see. Re-measure it rather than scaling it. `getEnergyFee` is a governance parameter and has already halved once, from 210 sun to 100, so this cost can move without the TRX price moving at all.
