...
Energy delegation is what makes the lower number safe. With energy staked and delegated to the float, a payout consumes delegated energy instead of burning TRX, and the fee no longer has to cover a burn. On Nile that cannot be *proved* — the test USDT contract sponsors its own energy, so every payout there reports `energy_fee: 0` whether anything is delegated or not — but Nile energy costs nothing either way, so this testnet charges $1 rather than pricing in a burn it does not pay. On mainnet the burn-covering fee stays until a real payout receipt shows sender-supplied energy; see [Redemptions](/clutch-treasury/redemptions).

The fee is bounded below by the smallest redemption a deployment allows. The treasury refuses a redemption that cannot cover its own fee, and that refusal reaches the user as a bare 502 rather than an explanation, so `APP_MIN_REDEMPTION_CLT` has to stay above `APP_REDEMPTION_FEE_USDT`. At $1 against the $5 floor the fee is a fifth of the smallest allowed redemption, falling to 1% at $100. At the burn-covering $5 it would be half of a $10 floor — which is what the on-chain payout costs, not a margin the treasury keeps.

Re-measure this rather than scaling it. `getEnergyFee` is a governance parameter and has already halved once, from 210 sun to 100, so the cost can move without the TRX price moving at all.
