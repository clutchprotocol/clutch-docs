...
The floor is **$5**, which is about twice what a sweep costs. Measured on 2026-09-10: a TRC-20 USDT transfer into an address that already holds USDT burns 64,285 units of energy, and the chain charged 100 sun per unit, so 6.43 TRX, or roughly $2.19 at a TRX price of $0.34. A sweep is always that cheaper shape, because the treasury address holds USDT from its first sweep onward. The doubling absorbs a moving TRX price and a moving `getEnergyFee`, which is a governance parameter and has halved once already.
